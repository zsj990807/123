...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.output_projection = nn.Linear(configs.d_model, self.c_out)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, edge_index):
        ...
        dec_out = self.decoder(dec_out, fused_enc_out, x_mask=None, cross_mask=None)  # [B, L, D]

        # Decoder 输出后 GAT + LSTM 融合
        B, L, D = dec_out.size()
        dec_out_flat = dec_out.view(B * L, D)              # [B*L, D]
        dec_out_gat = self.gat(dec_out_flat, edge_index)   # [B*L, D]
        dec_out_gat = dec_out_gat.view(B, L, D)            # [B, L, D]

        fused_dec_out = self.lstm_gated_fusion(dec_out, dec_out_gat)  # [B, L, D]

        return self.output_projection(fused_dec_out)
